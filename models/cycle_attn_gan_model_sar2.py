...
class CycleAttnGANModel_sar2(BaseModel):
    def name(self):
    ...
    def test(self):
        ...
        fake_B = self.netG_A.forward(self.real_C)
        ...
        rec_B = self.netG_A.forward(self.real_C)
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            ...
            self.idt_A = self.netG_A.forward(self.real_C)
        ...
        self.loss_G_A = self.criterionGAN(pred_fake, True)+0.5*self.criterionIdt(self.fake_B,self.real_B)
        # D_B(G_B(B))
        fake_A = self.netG_B.forward(self.real_B)
        self.attn_real_B = self.netA_B.forward(self.real_B)
        self.fake_A = self.mask_layer(fake_A, self.real_B, self.attn_real_B)
        pred_fake = self.netD_B.forward(self.fake_A)
        self.loss_G_B = self.criterionGAN(pred_fake, True)+0.5*self.criterionIdt(self.fake_A,self.real_A)
        # Forward cycle loss
        rec_A = self.netG_B.forward(self.fake_B)
        self.attn_fake_B = self.netA_B.forward(self.fake_B)
        self.rec_A = self.mask_layer(rec_A, self.fake_B, self.attn_fake_B)
